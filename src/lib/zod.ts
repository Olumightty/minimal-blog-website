...
export const newUserSchema = z.object({
  ...
  email: z
    ...
    .transform((email) => DOMPurify.sanitize(email)),
  password: z
    .string()
    ...
    .transform((password) => DOMPurify.sanitize(password)),
});
...
export const updateProfileSchema = z.object({
  ...
  website: z
    .string()
    .url()
    .transform((website) => DOMPurify.sanitize(website)),
  birthdate: z.string().transform((birthdate) => DOMPurify.sanitize(birthdate)),
  gender: z
    .enum(['male', 'female', 'prefer not to say'])
    .transform((gender) => DOMPurify.sanitize(gender)),
  social_links: z.array(
    z.object({
      platform: z
        .string()
        .min(1, { message: 'Invalid Platform' })
        .transform((platform) => DOMPurify.sanitize(platform)),
      url: z
        .string()
        .url()

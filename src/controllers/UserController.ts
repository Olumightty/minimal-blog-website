...
class UserController {
    UpdateProfile = async(req: Request, res: Response) => {
        ...
        // console.log('Hi')
        const validate = updateProfileSchema.safeParse({...updateData, social_links: JSON.parse(updateData.social_links)}); //zod validation
        if(!validate.success) {
            return res.status(400).json({error: 'Provided Credentials may be Invalid'})
        };
        ...
        try {
            ...
            const updateProfile = await Profile.findOneAndUpdate({user_id: id}, { ...profile, avatar, socialLinks: validate.data.social_links}, {new: true});
            if(!updateProfile) throw new Error('User profile not found, error has ocurred');

...
class AuthController {
    SignIn = async(req: Request, res: Response) => {
        ...
        if(!validateUserInput.success) throw new Error(validateUserInput.error.message);
        else{
            try {
                const user = await User.findOne({email: validateUserInput.data.email});
                if(!user) throw new Error('User doees not exists');
                if(!user.password) throw new Error('User was not created with Email/Password');
                const isValidPassword = await bcrypt.compare(validateUserInput.data.password, user.password!);
    ...
    SignUp = async(req: Request, res: Response) => {
        ...
        if(!validateUserInput.success) throw new Error(validateUserInput.error.message);
            ...
            try {
                ...
                const hashPassword = await bcrypt.hash(validateUserInput.data.password, Number(process.env.SALT_ROUNDS));
                if(!hashPassword) throw new Error('Could not hash password');
                const newUser = new User({name: validateUserInput.data.firstName + " " + validateUserInput.data.lastName, password: hashPassword, email: validateUserInput.data.email});
                await newUser.save();
    ...
    ValidateEmail = async(req: Request, res: Response) => {
        ...
        try {
            ...
            const validOtp = jwt.verify(user.verificationToken!, process.env.JWT_SECRET as string) as JwtPayload;
            if(!validOtp) throw new Error('This OTP is not valid anymore');
            if(validOtp.otp != otp) throw new Error('Wrong OTP');
            const updateUser = await User.updateOne({email: req.session.user!.email}, {$set: {verified: true}})
            if(!updateUser.acknowledged) throw new Error('Could not update verification status');
